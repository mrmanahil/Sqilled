...
import SubscribeWithOutCard from './subscribePayment/subscribeWithoutCard'
import SubscribeWithCard from './subscribePayment/subScribeWithCard';
...
const SaveFreeBooking = loader('../../graphql/search/saveFreeBooking.graphql');
...
const SearchDetails = (props) => {
			...
			if (hh >= 1) {
				...
				props.client.query({
					...
					if (response.data.checkAvailableBookingSlot.status === 'UNAVAILABLE') {
						...
						setSlotMessage(response.data.checkAvailableBookingSlot.status);
						setUnAvailabliltyData(response.data.checkAvailableBookingSlot.unavailable_time_slot);
						console.log('res',response.data.checkAvailableBookingSlot)
					} else {
						$(".error").empty();
						setSlotMessage(response.data.checkAvailableBookingSlot.status);
						setSlotAvail(false);
	...
	const onCloseModal = (data) => {
		if (data.status !== undefined && data.status === 'SUCCESS') {
			...
			console.log('sucss',data.message)
	...
	const onCloseModal2 = (data) => {
		if (data.status !== undefined && data.status === 'SUCCESS') {
			...
			if((localStorage.getItem('userType'))==="W"){
			setTimeout(() => {
				props.history.push('/my-subscribers');
			}, 2000);
		}else{
			setSuccessMsg(data.message);
		}
	...
	const handleFreeBooking = () => {
		$("#loadingDiv").show();
		props.client.mutate({
			mutation: SaveFreeBooking,
			variables: {
				"data": {
					"booking_date": bookDate,
					"start_time": (moment(new Date(startTime)).format('HH:mm:ss')),
					"end_time": (moment(new Date(endTime)).format('HH:mm:ss')),
					"author_id": authorId,
				   
				  }
			},
		}).then(response => {
			if (response.data.saveFreeBooking.status === 'SUCCESS') {
				setBooking(response.data.saveFreeBooking.message);
				onCloseModal3(response.data.saveFreeBooking);
				console.log('res',response.data.saveFreeBooking)
			  } else {
				setBooking( response.data.saveFreeBooking.message);
				console.log('res2',response.data.saveFreeBooking)
			  }
			  $("#loadingDiv").hide();
			}).catch(error => {
			  let errorMsg = commonFunctions.parseGraphQLErrorMessage(error);
			  setBooking( errorMsg );
			  console.log('res3', errorMsg)
			  $("#loadingDiv").hide();
			});
	}
			...
			<div className="profile-sec">
				<div className="container search-details-page">
					<div className="profile-form" id="scrollTop">
						...
						<a  className="update booking-btn" onClick={emailSubscription} data-toggle="modal" data-target="#exampleModalScrollable">Subscribe for monthly newsletter for $10 / month
						...
						<div className="profile_box">
							...
							{userType === 'W' &&
								<React.Fragment>
									...
									<div className="form-group">
										...
										<span className="video-profile" onClick={openVideoPopModal}>Profile Video</span>
									</div>
									<div className="form-group">
										<label>Hourly Rate </label>
										<span style={{textDecoration:'line-through'}}>{`${hourRate ? `$ ${hourRate}/hr`  : 'N/A'}`}</span>
										{'  '}
										<span>Free</span>
							...
							<div className="booking-box">
								...
								<div className="form-group set-time">
									...
									/>
									<div className="form-group success_msg" >
								...
								<div className="form-group" >
									<button disabled={!disableBtn} className="update booking-btn" onClick={handleFreeBooking} data-toggle="modal" data-target="#exampleModalScrollable">Book Now For Free</button>
								</div>
							</div>
							<Modal dialogClassName="modal_payment" aria-labelledby="example-custom-modal-styling-title" open={openModal2} onClose={onCloseModal2} closeOnOverlayClick={false}>
							<div className="stripe-details">
							{isCardAvail !== null
										&& (<React.Fragment>
												<SubscribeWithOutCard
												firstName={firstName}
												hourRate={hourRate}
												lastName={lastName}
							...
							<Modal dialogClassName="modal_payment" aria-labelledby="example-custom-modal-styling-title" open={openModal} onClose={onCloseModal} closeOnOverlayClick={false}>
								<div className="stripe-details">
									{isCardAvail !== null
										&& (<React.Fragment>
											<PaymentWithoutCard
						...
						{/* video profile modal */}
						<Modal open={openVideoModal} onClose={onCloseModal} closeOnOverlayClick={false}>
